...
const InvoicesTable: React.FC<InvoicesTableProps> = ({
  ...
  const { invoices } = useStore(); // Cambié customers a invoices
            ...
            {invoices.map((invoice, index) => (
              ...
              >
                ...
                <td className="py-4 text-left">
                     {customers.find((customer) => customer.Id === invoice.ClientId)?.Name}
                </td>
                <td className="py-4 text-left">{invoice.Date.toString()}</td>
                <td className="py-4 text-left">{invoice.Total}</td>
                <td className="py-4 pr-4 text-right">
                  <button
                    className="font-medium text-blue-600 dark:text-blue-500 hover:underline"
                    onClick={() => onClickEditModal(invoice)}
                  >
                    Editar
                  </button>
                </td>
